...
export const ExpenseModel = {
  async getExpenses(
    ...
    const result = await pool.query<ExpenseRecord>(query, params);
    return result.rows;
  },

  async getSummary(userId: number): Promise<ExpenseSummaryMetrics> {
    // 1. Total de egresos
    const totalQuery = `
    ...
    const totalEgresos = Number(totalRes.rows[0]?.total) || 0;

    // 2. Último egreso registrado
    const lastQuery = `
    ...
    const ultimoEgreso = Number(lastRes.rows[0]?.monto) || 0;
    ...
    const promedioMensual = Number(avgRes.rows[0]?.promedio) || (totalEgresos > 0 ? totalEgresos : 0);
  ...
  async getById(userId: number, expenseId: number): Promise<ExpenseRecord | null> {
    const query = `
      SELECT
        id,
        user_id,
    ...
    `;
    const result = await pool.query<ExpenseRecord>(query, [expenseId, userId]);
